...
- [x] **PitchAngle union widening now demo-blocking:** RESOLVED (lane 2, ~2:45 PM) — union widened with all 7 RE/MK angles in docs/CONTRACTS.md + types.ts + schemas.ts + frontend lib/types.ts (lockstep); lens packs carry the full `{angle, trigger, line}` menus.

## FINDINGS — lane 2 (content+contracts+render), 2026-06-12 ~2:50 PM
- [x] **Planted-catch boundary IMPLEMENTED:** judge live corpus = `brief.signals` + `sellerSignals(lens)` ONLY (nodes/judge.ts `buildEvidenceCorpus`) — brief prose never reaches the critic. Probed live on RE-1.
- [ ] **data/leads NAMESPACE COLLISION (flagged, resolved by convention, NOT silently):** lane 1's live scrape writes a raw-markdown cache to `data/leads/<domain>.json` (`{url,domain,scrapedAt,markdown}`) — it OVERWROTE the lindy planted-brief fixture. Convention now: **planted CompanyBrief fixtures live at `data/leads/<id>.brief.json`** (lindy.ai.brief.json, retell.ai.brief.json); RE/MK fixtures keep their names (handles are `fixture://` and should never live-scrape — if the orchestrator ever scrapes them, the cache writer will stomp the briefs). `data/scenarios/*.json` pointers updated. Whoever builds the cached-scenario loader must read `.brief.json` files and feed them in as the brief (they zCompanyBrief.parse cleanly; `_planted` markers strip at the boundary).
- [ ] **ORCH-1 (render-before-gate) NOT taken:** render still runs post-approve (sequencing unchanged — OPENUI-RENDER's documented fallback: receipt renders confirmatory after the gate). Receipt delivery = `GET /story/:leadId/receipt` `{openuiLang, slides}`; openuiLang null = visible "openui FAILED -> local slides" badge.
- [ ] **Judge calibration enforced in code (visible overrides, not silent):** fabricatedClaims non-empty -> grounding capped at 0.55; verdict recomputed from the contract rule (emit iff no fabricated + all axes >= 0.7). Drafter pitch_angle outside the lens menu -> coerced to menu[0] with a loud `[seam]` error line.

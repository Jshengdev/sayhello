...
function ReceiptShell({ company, headline, children }: { company: string; headline: string; children: ReactNode }) {
  /* fixture names can carry editor comments (`// …`) — never show them */
  const cleanCompany = company.split("//")[0].trim();
  return (
    <div className="ring-card rounded-xl bg-raised p-4">
      <header className="flex flex-wrap items-baseline justify-between gap-2">
        <p className="font-mono text-[10.5px] uppercase tracking-[0.14em] text-mute">
          verification receipt · {cleanCompany}
        </p>
...
function verdictTone(v: Verdict): { cls: string; mark: string; label: string } {
  /* plain language on screen (DEMO-FEEL item 6) — the verdict itself stays real */
  if (v === "FABRICATED") return { cls: "text-bad", mark: "⊘", label: "can't verify" };
  if (v === "CUT") return { cls: "text-mute", mark: "—", label: "cut" };
  return { cls: "text-good", mark: "✓", label: "proven" };
}
...
function TrajectoryChart({ points }: { points: LangValue[] }) {
  ...
  return (
    <div>
      <p className="font-mono text-[9px] uppercase tracking-[0.12em] text-faint">proof, draft by draft</p>
...
export function treeFromRun(run: StoryRun): ElementNode {
  ...
  for (const c of flagged) evidence.push(el("EvidenceItem", c, "No source found anywhere we looked — flagged, and cut.", null));
  for (const s of (brief?.signals ?? []).slice(0, 5)) {
  ...
  const headline =
    flagged.size > 0
      ? `${flagged.size} made-up claim${flagged.size > 1 ? "s" : ""} caught and cut`
      : "verified — every line traces to evidence";
  ...
  children.push(
    el("GateBlock", "Approve & get the report", "approve_story", `Proof ${finalG.toFixed(2)} — your call`),
  );

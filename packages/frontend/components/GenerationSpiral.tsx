...
export default function GenerationSpiral({
  ...
  /** controlled scrub target — a chip click lands the spiral on this draft */
  focus?: number | null;
  ...
  const [focusIdx, setFocusIdx] = useState(() => (typeof focus === "number" ? focus : 0));
  ...
  /* a chip click (controlled focus) turns the spiral to that draft. Rounded
     compare so the wheel/drag scrub never fights the controlled value. */
  useEffect(() => {
    if (typeof focus !== "number" || n === 0) return;
    const t = Math.max(0, Math.min(n - 1, focus));
    if (Math.round(targetRef.current) !== t) targetRef.current = t;
  }, [focus, n]);

  /* report the scrubbed focus up (the detail card under the spiral follows) */
  useEffect(() => {
        ...
        <span className="flex items-center gap-2">
          {n > 0 && focused && (
            <p className="m-0 font-mono text-[10px] text-mute">
              ...
              /{" "}
              <span className="font-numeral">{n}</span> · proof{" "}
              <span
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              data-testid="spiral-close"
              aria-label="Back to the garden"
              className="pill-emboss rounded-full bg-page px-2 py-[2px] font-mono text-[10px] text-mute transition-colors hover:text-ink"
            >
              ✕
            </button>
          )}
              ...
              {generations.map((g, i) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    tabIndex={-1}
                    aria-label={`Focus draft ${g.generation + 1}`}
                    ...
                    <span className="blur-up flex h-full flex-col p-3.5">
                      <span className="flex items-baseline justify-between">
                        <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-mute">
                          draft <b className="font-numeral text-[12px] text-ink">{g.generation + 1}</b>
                        </span>
                        <span
                          className={`font-mono text-[10px] font-medium ${grounded ? "text-good" : "text-bad"}`}
                        >
                          {grounded ? "proven ✓" : `⊘${g.fabricatedClaims.length} can't verify`}
                          {" · "}

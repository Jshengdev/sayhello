...
// Deploy seam: NEXT_PUBLIC_API_BASE / NEXT_PUBLIC_WS_URL are inlined at build time
// (Render points them at the backend service; locals keep the localhost defaults).
// NEXT_PUBLIC_API_URL kept as a legacy alias.
...
export function useStoryRun() {
  ...
  const startRun = useCallback(async (input: RunRequest) => {
    ...
    // zRunInput wants person targets NESTED as person:{linkedinUrl,xHandle} — top-level
    // fields get zod-stripped silently (V2 validator finding: UI runs logged person:false).
    const { linkedinUrl, xHandle, ...rest } = input;
    const body =
      linkedinUrl || xHandle
        ? {
            ...rest,
